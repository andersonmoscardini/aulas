...
public class PrimeiraAulaTest {
	...
	public void testSomaResultadoArgumentoNulo() {
		...
		esperado = Double.NaN;
		resultado = this.obj.somaTotal(valores);///esperado é 0
		comparacao = Double.compare(resultado, esperado);///esperado é 0
		Assert.assertEquals(
	...
	public void testSomaResultadoArgumentoArrayUnitario() {
		...
		valores[0] = Math.random();/// inicializando o valores eo random gera um valor aleatório
		esperado = valores[0];
		resultado = this.obj.somaTotal(valores); //chamada do metodo
		comparacao = Double.compare(resultado, esperado);
		Assert.assertEquals(
